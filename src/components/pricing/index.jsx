          ...
          {userType === "doctor" && currentPlans.length === 0 ? (
            <div className="text-center py-20">
              ...
              <p className="text-xl text-gray-600 max-w-2xl mx-auto">
                We're working on special pricing plans for healthcare
                professionals. Stay tuned for exclusive features designed
                specifically for medical practitioners.
              </p>
            </div>
          ) : (
            <div className="space-y-12 md:space-y-2">
              {planRows.map((row, rowIndex) => (
                <div
                  key={rowIndex}
                  className="grid grid-cols-1 sm:grid-cols-1 md:grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-8 justify-items-center"
                >
                  {row.map((plan) => (
                    <PricingCard
                      key={plan.id}
